...
def get_ghz_gate_tuples(num_qubits):
    ...
    """
    gate_tuples = [
        (instr.INSTR_INIT, [ii for ii in range(num_qubits)], "mono_qc"),
        (instr.INSTR_H, 0, "mono_qc"),
        *[
            (instr.INSTR_CNOT, ii, "mono_qc", ii + 1, "mono_qc")
...
def get_gate_tuples_to_create_cluster_state(topology):
    ...
    for edge in topology:
        qubit_index0 = edge[0]
        qubit_index1 = edge[1]
        gate_tuple = (instr.INSTR_CZ, qubit_index0, "mono_qc", qubit_index1, "mono_qc")
    ...
    hadamards = [
        (instr.INSTR_H, qubit_index, "mono_qc") for qubit_index in qubits_to_initialise
    ]
