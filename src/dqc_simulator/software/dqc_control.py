...
class InterpreterProtocol(NodeProtocol):
    ...
    def __init__(self, superprotocol, gate_tuples, node=None, name=None):
        ...
        # above):
        self.single_qubit_subroutines = {
            "logged": self._logged_instr,
        }
        self.remote_gate_variants = {
            "cat": self._cat_subgenerator,
    ...
    def add_phys_layer(self, physical_layer_protocol):
        ...
        self.add_subprotocol(physical_layer_protocol, name="physical_layer_protocol")

    def _flexi_program_apply(
        self, qprogram, gate_instr, qubit_indices, gate_op, output_key="last"
    ):
        """
        ...
        if gate_op is None:
            ...
            qprogram.apply(
                gate_instr, qubit_indices, operator=gate_op, output_key=output_key
            )

    def _request_entanglement(
        self,
        role,
        other_node_name,
        comm_qubit_indices,
    ...
    def _cat_entangle(
        self, program, other_node_name, data_or_tele_qubit_index, classical_comm_port
    ):
        ...
        if len(self.node.qmemory.comm_qubits_free) == 0:
            ...
            if not self.node.qmemory.ebit_ready:  # if there is no ebit ready:
                yield from self._request_entanglement(
                    "client", other_node_name, [comm_qubit_index]
                )
            program.apply(
                instr.INSTR_CNOT, [data_or_tele_qubit_index, comm_qubit_index]
            )
            program.apply(instr.INSTR_MEASURE, [comm_qubit_index], output_key="ma")
    ...
    def _cat_correct(self, program, other_node_name, classical_comm_port):
        ...
        if len(self.node.qmemory.comm_qubits_free) == 0:
            ...
            if not self.node.qmemory.ebit_ready:  # if there is no ebit ready:
                yield from self._request_entanglement(
                    "server", other_node_name, [comm_qubit_index]
                )
            ...
            if meas_result == 1:
                ...
                raise ValueError(
                    "Measurement result must be an integer with "
    ...
    def _cat_disentangle_start(self, program, comm_qubit_index, classical_comm_port):
    ...
    def _cat_disentangle_end(
        self, program, classical_comm_port, data_or_tele_qubit_index
    ):
        yield self.await_port_input(classical_comm_port)
        (meas_result,) = classical_comm_port.rx_input().items
        if meas_result == 1:
            ...
            raise ValueError(
                "Measurement result must be an integer with value"
    ...
    def _bsm(
        self, program, data_or_tele_qubit_index, other_node_name, classical_comm_port
    ):
        ...
        if len(self.node.qmemory.comm_qubits_free) == 0:
            ...
            # block
            if data_or_tele_qubit_index == -1 and len(
                self.node.qmemory.comm_qubits_free
            ...
            program.apply(instr.INSTR_MEASURE, [comm_qubit_index], output_key="m1")
            # =============================================================================
            ...
            # =============================================================================
            program.apply(
                instr.INSTR_MEASURE, [data_or_tele_qubit_index], output_key="m2"
            )
            yield self.node.qmemory.execute_program(program)
            self._gate_tuples_evaluated[-1].extend(self._local_gate_tuples_pending)
            # re-initialising self._local_gate_tuples_pending
            ...
            if data_or_tele_qubit_index in self.node.qmemory.comm_qubit_positions:
                ...
                # measured
                self.node.qmemory.comm_qubits_free = (
                    self.node.qmemory.comm_qubits_free + [data_or_tele_qubit_index]
                )
    ...
    def _handle_meas_results4tp_correct(self, meas_results, index, program):
        error_msg = (
            "All elements of meas_results must be integers with value"
    ...
    def _tp_correct(
        ...
        if reserve_comm_qubit:
            ...
            reserved_index = self.node.qmemory.comm_qubits_free.index(comm_qubit_index)
        ...
        if swap_commNdata:
            ...
            # =============================================================================
            program.apply(
                instr.INSTR_SWAP, [comm_qubit_index, data_or_tele_qubit_index]
            )
            self._handle_meas_results4tp_correct(
                meas_results, data_or_tele_qubit_index, program
            )
        else:
            self._handle_meas_results4tp_correct(
                meas_results, comm_qubit_index, program
            )
        yield self.node.qmemory.execute_program(program)
        self._gate_tuples_evaluated[-1].extend(self._local_gate_tuples_pending)
        # re-initialising self._local_gate_tuples_pending
    ...
    def _cat_subgenerator(
        ...
        if role == "entangle":
            evexpr_or_program = yield from self._cat_entangle(
                program, other_node_name, data_or_tele_qubit_index, classical_comm_port
            )
            ...
            evexpr_or_program = yield from self._cat_disentangle_start(
                program, comm_qubit_index, classical_comm_port
            )
            program = evexpr_or_program

        elif role == "disentangle_end":
            evexpr_or_program = yield from self._cat_disentangle_end(
                program, classical_comm_port, data_or_tele_qubit_index
            )
            program = evexpr_or_program

        else:
            raise ValueError("final element of gate_tuple is not valid role")
        return (comm_qubit_index, program)

    def _tp_subgenerator(
        self,
        role,
        program,
        other_node_name,
        ...
        if role == "bsm":
            evexpr_or_program = yield from self._bsm(
                program, data_or_tele_qubit_index, other_node_name, classical_comm_port
            )
            program = evexpr_or_program

        elif role == "correct":
            evexpr_or_tuple = yield from self._tp_correct(
                program,
                other_node_name,
                classical_comm_port,
            ...
            raise ValueError(f"final element, {role} of gate_tuple is not a valid role")
        return (comm_qubit_index, program)
    ...
    def _logged_instr(self, program, gate_tuple):
        ...
        self._local_gate_tuples_pending = []
        program = QuantumProgram()
        self._flexi_program_apply(
            program, gate_instr, ancilla_qubit_index, gate_op, output_key="result"
        )
        yield self.node.qmemory.execute_program(program)
        (result,) = program.output["result"]
        self.send_signal(
            QDCSignals.RESULT_PRODUCED, result=(result, ancilla_qubit_index)
        )
    ...
    def _handle_remote_gate_primitive(self, program, gate_tuple, comm_qubit_index):
        ...
        node_names.sort()
        classical_connection_port_name = self.node.connection_port_name(
            other_node_name, label="classical"
        )
        classical_conn_port = self.node.ports[classical_connection_port_name]
        evexpr_or_variables = yield from self.remote_gate_variants[scheme](
            role,
            program,
            other_node_name,
            data_or_tele_qubit_index,
    ...
    def _handle_local_two_qubit_gate(self, program, gate_tuple, comm_qubit_index):
        gate_instr, gate_op = self._get_gate_instruction_and_op(gate_tuple)
        ...
        if qubit_index0 == -1:
            ...
            # section above
        self._flexi_program_apply(
            program, gate_instr, [qubit_index0, qubit_index1], gate_op
        )
        self._local_gate_tuples_pending.append(gate_tuple)
        # self._local_gate_tuples_pending will be added to
        # self._gate_tuples_evaluated whenever a program is run,
    ...
    def _run_time_slice(self, gate_tuples4time_slice):
        ...
        for gate_tuple in gate_tuples4time_slice:
            if len(gate_tuple) == 2:  # if single-qubit gate:
                ...
                isinstance(gate_tuple[-1], str)
                and gate_tuple[-2] in self.remote_gate_variants
            ):  # if primitive for remote
                # gate:
                # The remote gates in this block will use different
                # comm-qubits because logical gates using the same
                # comm-qubit will occur in different time slices or will
                ...
                evexpr_or_variables = yield from self._handle_remote_gate_primitive(
                    program, gate_tuple, comm_qubit_index
                )
                # past this point evexpr_or_variables will be the
            ...
            # to change compiler)
                ...
                self._handle_local_two_qubit_gate(program, gate_tuple, comm_qubit_index)
    ...
    def raise_error_if_not_all_gates_executed(self):
        ...
        if self._gate_tuples_evaluated != self.gate_tuples:
            index_of_last_time_slice_evaluated = len(self._gate_tuples_evaluated) - 1
    ...
    def run(self):
        if "physical_layer_protocol" not in self.subprotocols or not isinstance(
            self.subprotocols["physical_layer_protocol"], PhysicalLayerProtocol
        ):
            raise ValueError(
                f"{self.name} requires a physical layer protocol, "
        ...
        for time_slice in range(self._max_num_time_slices):
            yield self.await_signal(
                self.superprotocol, signal_label=self.start_time_slice_label
            )
...
class DQCMasterProtocol(LocalProtocol):
    ...
    def __init__(
        ...
        for node_name in self.qpu_op_dict:
            ...
            if isinstance(node.qmemory, QPU):
                ...
                self.physical_layer_protocol = physical_layer_protocol_class(node=node)
                ...
                self.add_subprotocol(qpu_protocol, name=f"{node_name}_OS")
    ...
    def check_quantum_circuit_finished(self):
        ...
        for qpu_name in self.qpu_op_dict:
            self.subprotocols[f"{qpu_name}_OS"].raise_error_if_not_all_gates_executed()
