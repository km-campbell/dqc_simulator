...
def bisect_circuit(dqc_circuit, comm_qubits_per_node=2):
    ...
    # helper functions:
    def _assign_qubit_to_node(qubit_index, qreg_name, node1starting_index):
        starting_index = (
            dqc_circuit.qregs[qreg_name]["starting_index"] + comm_qubits_per_node
        )
        updated_qubit_index = qubit_index + starting_index
        node_name = "node_0"
        if updated_qubit_index >= node1starting_index:
            updated_qubit_index = (
                updated_qubit_index - node1starting_index + comm_qubits_per_node
            )
            node_name = "node_1"
    ...
    for gate_spec in dqc_circuit.ops:
        ...
        updated_qubit_index, node_name = _assign_qubit_to_node(
            qubit_index1, qreg_name1, node1starting_index
        )
        ...
        if len(gate_spec) >= 5:
            ...
            updated_qubit_index, node_name = _assign_qubit_to_node(
                qubit_index2, qreg_name2, node1starting_index
            )
...
def first_come_first_served_qubits_to_qpus(gate_tuples, qpu_nodes):
    ...
    if init_gate_tuple[0] is not instr.INSTR_INIT:
        raise ValueError(
            "first instruction in gate_tuples must initialise "
    ...
    for qpu_node in qpu_nodes:
        ...
        old_indices_assigned_to_qpu = [ii for ii in range(starting_index, end_index)]
        ...
        old2new = {
            old_indices_assigned_to_qpu[ii]: (
...
# input
def partition_gate_tuples(
    gate_tuples, network, scheme, old_to_new_lookup, proc_qubit_allocation4each_qpu
):
    """
    ...
    def _split_up_initial_init_instruction(gate_tuples, proc_qubit_allocation4each_qpu):
        """
        ...
        """
        if gate_tuples[0][0] is instr.INSTR_INIT and isinstance(
            gate_tuples[0][1], list
        ):
            ...
            for node_name in proc_qubit_allocation4each_qpu:
                init_instructions.append(
                    (
                        instr.INSTR_INIT,
                        proc_qubit_allocation4each_qpu[node_name],
                        node_name,
                    )
                )
        else:
            raise ValueError(
                "First gate must be an initialisation. The gate"
    ...
    partitioned_init_instructions = _split_up_initial_init_instruction(
        gate_tuples, proc_qubit_allocation4each_qpu
    )
    ...
    for gate_tuple in gate_tuples[1:]:
        ...
        if len(new_gate) > 3 and any(
            new_gate[ii] != new_gate[ii + 2] for ii in range(2, len(new_gate) - 2, 2)
        ):
    ...
    partitioned_gate_tuples = partitioned_init_instructions + partitioned_gate_tuples
