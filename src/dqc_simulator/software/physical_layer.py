...
class PhysicalLayerProtocol(NodeProtocol):
    ...
    def deterministic(self):
        ...
        if not isinstance(self._deterministic, bool):
            raise TypeError(
                "The deterministic property must have type bool. "
    ...
    def deterministic(self, value):
        raise TypeError(
            "deterministic is a read-only property - its valuecannot be set by the user"
        )
    ...
    def run_link_layer(self):
        ...
        """
        yield self.await_signal(self.superprotocol, signal_label=self.ent_request_label)
        ...
        signal_results = self.superprotocol.get_signal_result(self.ent_request_label)
        ...
        self.classical_connection_port_name = self.node.connection_port_name(
            self.other_node_name, label="extra_classical"
        )
        self.classical_conn_port = self.node.ports[self.classical_connection_port_name]
        self.entangling_connection_port_name = self.node.connection_port_name(
            self.other_node_name, label="entangling"
        )
        self.ent_conn_port = self.node.ports[self.entangling_connection_port_name]
    ...
    def handshake(self):
        ...
        while True:
            if self.role == "client":
                ...
                evexpr = yield (
                    self.await_port_input(self.classical_conn_port)
                ...
                if evexpr.first_term.value:
                    ...
                    # protocol:
                        ...
                        raise ValueError(
                            "The message must be a str with value "
                            '"ENT_READY_LABEL" or '
...
class MidpointHeraldingProtocol(ProbabilisticEntanglingProtocol):
    ...
    def entangle_comm_qubits(self):
        while True:  # will be broken with break command on successful
            ...
            # comm-qubits
            emit_program.apply(instr.INSTR_INIT, qubit_indices=self.comm_qubit_indices)
            ...
            if len(self.comm_qubit_indices) == 1:
                ...
                raise ValueError(
                    "There are too many comm-qubit indices. "
                    "Currently, only one comm-qubit can be used here."
                    "It is assumed that there is one photon emission"
                    " at a time."
                )
            emit_program.apply(
                instr.INSTR_EMIT, qubit_indices=[comm_qubit_index, photon_index]
            )
            # =============================================================================
            ...
            if bsm_outcome.success:
                ...
                if self.role == "server":
                    ...
                    program.apply(instr.INSTR_X, qubit_indices=comm_qubit_index)
                    if bsm_outcome.bell_index == ks.BellIndex.PSI_MINUS:
                        program.apply(instr.INSTR_Z, qubit_indices=comm_qubit_index)
