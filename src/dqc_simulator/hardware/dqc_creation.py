...
from netsquid.nodes import Node, DirectConnection, Network
...
def link_2_qpus(
    ...
    **kwargs4create_entangling_link,
    ...
    if (
        create_entangling_link is None
        or create_entangling_link == create_black_box_central_source_entangling_link
    ):
        ...
        create_entangling_link = create_wrapper_with_some_args_fixed(
            create_black_box_central_source_entangling_link,
            args2fix,
    ...
    if want_classical_2way_link:
        ...
        create_classical_fibre_link(
            network, node_a, node_b, length=node_distance, label="classical"
        )
    ...
    if want_extra_classical_2way_link:
        ...
        # free-space classical connection instead.
        create_classical_fibre_link(
            network, node_a, node_b, length=node_distance, label="extra_classical"
        )

    if want_entangling_link:
        create_entangling_link(network, node_a, node_b, **kwargs4create_entangling_link)
...
def create_dqc_network(
    ...
    **kwargs :
    ...
    kwargs4qproc, kwargs4create_entangling_link = filter_kwargs4internal_functions(
        [custom_qpu_func, create_entangling_link], kwargs
    ).values()
    ...
    if quantum_topology is None and classical_topology is None:
        ...
        if quantum_topology is not None:
            if (quantum_topology[-1])[1] > (len(network.nodes) - 1):
                ...
                nodes_needed = (quantum_topology[-1])[1] - (len(network.nodes) - 1)
                extra_nodes = _create_qpus(
                    num_qpus2create=nodes_needed, starting_index=num_qpus
                )
                network.add_nodes(extra_nodes)
                warnings.warn(
                    "WARNING: num_qpus was overwritten because there"
        ...
        if classical_topology is not None:  # if classical_topology is not None
            if (classical_topology[-1])[1] > (len(network.nodes) - 1):
                ...
                nodes_needed = (classical_topology[-1])[1] - (len(network.nodes) - 1)
                extra_nodes = _create_qpus(
                    num_qpus2create=nodes_needed, starting_index=num_qpus
                )
                network.add_nodes(extra_nodes)
...
class DQC(Network):
    ...
    **kwargs
    ...
    def __init__(
        ...
        want_extra_classical_link=True,
        name="DQC",
        nodes=None,
        ...
        **kwargs,
        ...
        kwargs4each_class = filter_kwargs4internal_functions(classes, kwargs)
        ...
        for pair in quantum_topology:
            entangling_connection = entangling_connection_class(
                **kwarg_types[entangling_connection_class]
            )
            label = "entangling"
    ...
    def _get_classical_connection(self, classical_connection_class, kwarg_types):
        if classical_connection_class is None:
            ...
            return classical_connection_class(**kwarg_types[classical_connection_class])
