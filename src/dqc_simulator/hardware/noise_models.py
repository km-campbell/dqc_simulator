...
def apply_analytical_depolarisation2dm(qubits, p_error):
    ...
    if isinstance(qubits, ns.qubits.qubit.Qubit):
        ...
        while ii < (len(qubits) - 1):
            ...
            if qubit.qstate != nxt_qubit.qstate:
                raise Exception(
                    f"Qubits {qubit.name} and {nxt_qubit.name} do"
    ...
    noise_mat = np.kron(partialtrace(ideal_dm, indices4subspace), np.eye(subspace_dim))
    # =============================================================================
    ...
    indices4complement_space = list(
        set(indices4entire_state_list) - set(indices4subspace)
    )
...
class AnalyticalDepolarisationModel(QuantumErrorModel):
    ...
    def error_operation(self, qubits, delta_time=0, **kwargs):
        ...
        if self.time_independent:  # for adding errors to quantum
            ...
            for qubit in qubits:
                prob_error = 1.0 - np.exp(-delta_time * 1e-9 * self.p_error)
    ...
    def error_operation(self, qubits, delta_time=0, **kwargs):
        ...
        for qubit in qubits:
            if qubit is not None:
                qapi.apply_pauli_noise(
                    qubit, (1 - self.error_probability, self.error_probability, 0, 0)
                )
...
class NDimDepolarNoiseModel(QuantumErrorModel):
    ...
    def error_operation(self, qubits, delta_time=0, **kwargs):
        ...
        paulis = [
            ft.reduce(lambda a, b: a ^ b, tup)
            for tup in it.product([ops.I, ops.X, ops.Y, ops.Z], repeat=len(qubits))
        ]

        # Apply Pauli strings to relevant qubits
        p = self.error_probability / num_paulis
        qapi.stochastic_operate(
            qubits, paulis, p_weights=tuple([1 - num_errors * p] + [p] * num_errors)
        )
