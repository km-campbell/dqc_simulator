...
from dqc_simulator.qlib.gates import (
    INSTR_ARB_GEN,
    INSTR_CH,
    INSTR_CT,
    INSTR_T_DAGGER,
...
class QPU(QuantumProcessor):
    ...
    def __init__(
        ...
        mem_pos_types, num_photon_spaces = self._get_mem_pos_types_N_num_photons(
            num_positions, num_comm_qubits
        )
        self.mem_pos_types = mem_pos_types
        if comm_qubit_mem_noise_model or processing_qubit_mem_noise_model is not None:
        ...
        self.processing_qubit_positions = self.get_positions_matching_type("processing")
        self.photon_positions = self.get_positions_matching_type("photon")
        self.num_real_positions = self.num_positions - len(self.photon_positions)
    ...
    def _get_mem_noise_models(
        ...
        for mem_pos_type in mem_pos_types:
            if mem_pos_type == "comm":
                ...
                raise ValueError(
                    f'"{mem_pos_type}" is not of one of the allowed '
...
def create_processor(
    ...
    physical_instructions = [
        PhysicalInstruction(
            instr.INSTR_INIT, duration=3, parallel=False, toplogy=[2, 3, 4, 5, 6]
        ),
        PhysicalInstruction(instr.INSTR_H, duration=1, parallel=True, topology=None),
        PhysicalInstruction(
            instr.INSTR_X, duration=x_gate_duration, parallel=True, topology=None
        ),
        PhysicalInstruction(instr.INSTR_Z, duration=1, parallel=True, topology=None),
        PhysicalInstruction(instr.INSTR_S, duration=1, parallel=True, topology=None),
        PhysicalInstruction(instr.INSTR_CNOT, duration=4, parallel=True, topology=None),
        PhysicalInstruction(INSTR_ARB_GEN(alpha, beta), duration=4, parallel=False),
        PhysicalInstruction(INSTR_CH, duration=4, parallel=True, topology=None),
        PhysicalInstruction(INSTR_CT, duration=4, parallel=True, topology=None),
        PhysicalInstruction(instr.INSTR_CS, duration=4, parallel=True, topology=None),
        PhysicalInstruction(
            instr.INSTR_MEASURE,
            duration=7,
            parallel=False,
        ...
        PhysicalInstruction(
            instr.INSTR_DISCARD, duration=3, parallel=False, toplology=[0, 1]
        ),
        PhysicalInstruction(
            instr.INSTR_SWAP, duration=12, parallel=True, topology=None
        ),
        PhysicalInstruction(instr.INSTR_T, duration=1, parallel=True, topology=None),
        PhysicalInstruction(INSTR_T_DAGGER, duration=1, parallel=True, topology=None),
    ]
    memory_noise_model = DepolarNoiseModel(depolar_rate=depolar_rate)
    processor = QuantumProcessor(
        name,
        num_positions=num_positions,
        mem_noise_models=([memory_noise_model] * num_positions),
...
def _phys_instructions_4_standard_lib_gates_and_convenience_ops(
    ...
    physical_instructions = [
        ...
        PhysicalInstruction(INSTR_ARB_GEN(alpha, beta), duration=3, parallel=True),
        ...
        PhysicalInstruction(
            INSTR_SINGLE_QUBIT_NEGLIGIBLE_TIME, duration=3, parallel=True, topology=None
        ),
        PhysicalInstruction(
            INSTR_TWO_QUBIT_NEGLIGIBLE_TIME, duration=3, parallel=True, topology=None
        ),
    ]
...
def create_qproc_with_analytical_noise_ionQ_aria_durations_N_standard_lib_gates(
    ...
    # creating processor for all Nodes
    physical_instructions = _phys_instructions_4_standard_lib_gates_and_convenience_ops(
        cnot_depolar_model,
        single_qubit_gate_noise_model,
        meas_noise_model,
        num_comm_qubits,
    ...
    qprocessor.add_composite_instruction(
        instr.INSTR_SWAP,
        [
...
def create_noisy_qpu(
    ...
    .. [1] M. Nielsen and I. Chuang, Quantum Computation and Quantum
        Information, 10th ed. (Cambridge University Press, 2010).
    """
    cnot_depolar_model = NDimDepolarNoiseModel(
        p_depolar_error_cnot, time_independent=True
    )
    ...
    single_qubit_gate_noise_model = DepolarNoiseModel(
        single_qubit_gate_error_prob, time_independent=True
    )
    # creating processor for all Nodes
    physical_instructions = _phys_instructions_4_standard_lib_gates_and_convenience_ops(
        cnot_depolar_model,
...
class NoisyQPU(QPU):
    ...
    def __init__(
        ...
        self.add_composite_instruction(
            instr.INSTR_SWAP,
            [
                (instr.INSTR_CNOT, (0, 1)),
                (instr.INSTR_CNOT, (1, 0)),
