...
def create_classical_fibre_link(network, node_a, node_b, length, label="classical"):
    """
    ...
    connection = DirectConnection(
        ...
        channel_BtoA=ClassicalChannel(
            "Channel_B2A", length=length, models={"delay_model": FibreDelayModel()}
        ),
    )
    ...
    node_a_port_name = node_a.connection_port_name(node_b.name, label=label)
    node_b_port_name = node_b.connection_port_name(node_a.name, label=label)
...
class BlackBoxEntanglingQsourceConnection(Connection):
    ...
    def __init__(
        self, delay, state4distribution, name="BlackBoxEntanglingQsourceConnection"
    ):
        super().__init__(name=name)
        qchannel_qsource2A = QuantumChannel("qchannel_qsource2A", delay=delay)
        qchannel_qsource2B = QuantumChannel("qchannel_qsource2B", delay=delay)
        cchannelA2qsource_trigger = ClassicalChannel(
            "cchannel_A2qsource_trigger", delay=0
        )
        ...
        # distribution time.
        # TO DO: revisit this
        cchannelB2qsource_trigger = ClassicalChannel(
            "cchannel_B2qsource_trigger", delay=0
        )
        # TO DO: revisit delay
        # as commented above
        qsource = QSource(
        ...
        self.add_subcomponent(qchannel_qsource2A, forward_output=[("A", "recv")])
        self.add_subcomponent(qchannel_qsource2B, forward_output=[("B", "recv")])
        self.add_subcomponent(cchannelA2qsource_trigger, forward_input=[("A", "send")])
        self.add_subcomponent(cchannelB2qsource_trigger, forward_input=[("B", "send")])
        ...
        qsource.ports["trigger"].connect(cchannelA2qsource_trigger.ports["recv"])
        qsource.ports["extra_trigger"].connect(cchannelB2qsource_trigger.ports["recv"])
...
class ProbabilisticQSourceConnection(Connection):
    ...
    def __init__(
        self, delay, states4distribution, name="ProbabilisticQsourceConnection"
    ):
        super().__init__(name=name)
        qchannel_qsource2A = QuantumChannel("qchannel_qsource2A", delay=delay)
        qchannel_qsource2B = QuantumChannel("qchannel_qsource2B", delay=delay)
        ...
        qsource = QSource(
            "qsource",
            StateSampler(
                [state[0] for state in states4distribution],
...
def create_black_box_central_source_entangling_link(
    network, node_a, node_b, state4distribution, ent_dist_rate=0
):
    """
    ...
    elif ent_dist_rate == 0.0:
        ent_dist_time = 0
    else:
        raise ValueError(
            f"{ent_dist_rate} is not a valid entanglement "
    ...
    connection = BlackBoxEntanglingQsourceConnection(
        delay=ent_dist_time, state4distribution=state4distribution
    )
    # generating names obeying netsquid naming conventions for port on QPUs that
    # connect a Node to a connection but using the (unique) node name as the ID
    node_a_port_name = node_a.connection_port_name(node_b.name, label="entangling")
    node_b_port_name = node_b.connection_port_name(node_a.name, label="entangling")
    network.add_connection(
        node_a,
        node_b,
        connection=connection,
...
def create_bb_elink(network, node_a, node_b, state4distribution, ent_dist_rate=0):
    """
    ...
    create_black_box_central_source_entangling_link(
        network, node_a, node_b, state4distribution, ent_dist_rate=ent_dist_rate
    )


def create_probabilistic_qsource_connection(
    network, node_a, node_b, states4distribution, ent_dist_rate=0
):
    """
    Sets up an abstract entangling link between QPUs.
    ...
    connection = ProbabilisticQSourceConnection(
        delay=ent_dist_time, states4distribution=states4distribution
    )
    # generating names obeying netsquid naming conventions for port on QPUs that
    # connect a Node to a connection but using the (unique) node name as the ID
    node_a_port_name = node_a.connection_port_name(node_b.name, label="entangling")
