...
from dqc_simulator.qlib.gates import INSTR_ARB_GEN, INSTR_CH, INSTR_CT, INSTR_T_DAGGER
...
class TestAnalyticalDepolarisationModelWorksOnGates(unittest.TestCase):
    def test_entangling_gate_with_10_percent_error(self):
        ...
        physical_instructions = [
            PhysicalInstruction(
                instr.INSTR_INIT, duration=3, parallel=False, toplogy=None
            ),
            PhysicalInstruction(
                instr.INSTR_H, duration=1, parallel=False, topology=None
            ),
            PhysicalInstruction(
                instr.INSTR_X, duration=x_gate_duration, parallel=False, topology=None
            ),
            PhysicalInstruction(
                instr.INSTR_Z, duration=1, parallel=False, topology=None
            ),
            PhysicalInstruction(
                instr.INSTR_S, duration=1, parallel=False, topology=None
            ),
            PhysicalInstruction(
                instr.INSTR_CNOT,
                duration=4,
            ...
            PhysicalInstruction(
                instr.INSTR_CS, duration=4, parallel=False, topology=None
            ),
            PhysicalInstruction(
                instr.INSTR_MEASURE,
                duration=7,
            ...
            PhysicalInstruction(
                instr.INSTR_SWAP, duration=12, parallel=False, topology=None
            ),
            PhysicalInstruction(
                instr.INSTR_T, duration=1, parallel=False, topology=None
            ),
            PhysicalInstruction(
                INSTR_T_DAGGER, duration=1, parallel=False, topology=None
            ),
        ]
    ...
    def test_entangling_gate_with_extra_qubit(self):
        ...
        desired_state = np.diag(
            np.array(
                [
                    (2 - p_depolar_error_cnot) / 4,
                    0,
                    p_depolar_error_cnot / 4,
                    0,
                    p_depolar_error_cnot / 4,
                    0,
                    (2 - p_depolar_error_cnot) / 4,
    ...
    def test_recover_ideal_case_when_error_0(self):
        ...
        (qubit1,) = processor.pop(1)
        (qubit2,) = processor.pop(2)
        desired_state = alpha * np.kron(ks.s0, np.kron(ks.s0, ks.s0)) + beta * np.kron(
            ks.s1, np.kron(ks.s1, ks.s0)
        )
        fidelity = qapi.fidelity([qubit0, qubit1, qubit2], desired_state)
        self.assertAlmostEqual(fidelity, 1.00000, 5)
...
class TestAnalyticalDepolarisationModelCanApplyMemoryDecoherence(unittest.TestCase):
    ...
    def test_can_degrade_qubit_over_10s_with_error_rate_0point1Hz(self):
        ...
        physical_instructions = [
            PhysicalInstruction(
                instr.INSTR_INIT, duration=0, parallel=False, toplogy=None
        ...
        desired_state = np.array([[(1 + np.exp(-1)) / 2, 0], [0, (1 - np.exp(-1)) / 2]])
    ...
    def test_can_give_correct_result4_10_qubits(self):
        ...
        single_qubit_desired_state = np.array(
            [[(1 + np.exp(-1)) / 2, 0], [0, (1 - np.exp(-1)) / 2]]
        )
        ten_qubit_desired_state = ft.reduce(np.kron, [single_qubit_desired_state] * 10)
    ...
    def test_decoherence_starts_after_initialisation(self):
        ...
        physical_instructions = [
            PhysicalInstruction(
                instr.INSTR_INIT, duration=10**9, parallel=False, toplogy=None
            ),  # artificially long for
            PhysicalInstruction(
                instr.INSTR_H, duration=1, parallel=False, topology=None
            ),
            PhysicalInstruction(
