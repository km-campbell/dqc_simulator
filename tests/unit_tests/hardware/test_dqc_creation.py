...
from dqc_simulator.hardware.dqc_creation import link_2_qpus, create_dqc_network
...
class Test_link_2_qpus(unittest.TestCase):
    ...
    def setUp(self):
        ns.sim_reset()
        self.qpu0 = Node("qpu0", qmemory=QuantumMemory("qmemory", num_positions=3))
        self.qpu1 = Node("qpu1", qmemory=QuantumMemory("qmemory", num_positions=3))
        self.qpu0port_name = self.qpu0.connection_port_name(
            self.qpu1.name, label="entangling"
        )
        self.qpu1port_name = self.qpu1.connection_port_name(
            self.qpu0.name, label="entangling"
        )
    ...
    def test_can_create_only_entangling_link(self):
        ...
        with self.subTest(msg="wrong type of connection"):
            self.assertIsInstance(
                list(self.network.connections.values())[0],
    ...
    def test_can_create_only_classical_link(self):
        ...
        with self.subTest(msg="wrong type of connection"):
            self.assertIsInstance(
                list(self.network.connections.values())[0], DirectConnection
            )
...
class Test_create_dqc_network(unittest.TestCase):
    ...
    def test_can_create_only_classical_link(self):
        ...
        with self.subTest(msg="wrong type of connection"):
            self.assertIsInstance(
                list(network.connections.values())[0], DirectConnection
            )
    ...
    def test_can_create_only_entangling_link(self):
        ...
        with self.subTest(msg="wrong type of connection"):
            self.assertIsInstance(
                list(network.connections.values())[0],
                BlackBoxEntanglingQsourceConnection,
            )
