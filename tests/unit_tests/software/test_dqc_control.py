...
class TestDqcMasterProtocol(unittest.TestCase):
    ...
    def test_ent_distribution(self):
        ...
        desired_state = np.sqrt(1/2) * (np.kron(ks.s0, ks.s0)
                                        + np.kron(ks.s1, ks.s1))
    ...
    def test_multiple_ent_distributions(self):
        ...
        with self.subTest('First pair in wrong state'):
            fidelity = qapi.fidelity([node0_qubits[0], node1_qubits[0]],
                                     desired_state)
            self.assertAlmostEqual(fidelity, 1.)
        # Testing second pair
        with self.subTest('Second pair in wrong state'):
            fidelity = qapi.fidelity([node0_qubits[1], node1_qubits[1]],
                                     desired_state)
            self.assertAlmostEqual(fidelity, 1.)
        # Testing third pair
        with self.subTest('Third pair in wrong state'):
            fidelity = qapi.fidelity([node0_qubits[2], node1_qubits[2]],
                                     desired_state)
            self.assertAlmostEqual(fidelity, 1.)
